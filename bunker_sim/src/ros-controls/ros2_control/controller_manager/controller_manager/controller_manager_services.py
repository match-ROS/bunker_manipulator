...
def set_controller_parameters(
    ...
    # NOTE: This must preserve the parameter type.
    # Previously we always converted to string_value=str(value). That corrupts lists like
    # ['a.yaml', 'b.yaml'] into a single string "['a.yaml', 'b.yaml']", which later gets
    # interpreted as a bogus params file path by controller_manager.
    if isinstance(parameter_value, bool):
        parameter_string = str(parameter_value)
        parameter.value = get_parameter_value(bool_value=parameter_value)
    elif isinstance(parameter_value, int) and not isinstance(parameter_value, bool):
        parameter_string = str(parameter_value)
        parameter.value = get_parameter_value(integer_value=parameter_value)
    elif isinstance(parameter_value, float):
        parameter_string = str(parameter_value)
        parameter.value = get_parameter_value(double_value=parameter_value)
    elif isinstance(parameter_value, (list, tuple)):
        # Most controller_manager helper parameters are string arrays (e.g., params_file,
        # node_options_args, fallback_controllers).
        string_array = [str(v) for v in parameter_value]
        parameter_string = str(string_array)
        # Fix for rclpy.parameter.get_parameter_value which only accepts string_value
        # We need to construct the ParameterValue message manually for string arrays
        from rcl_interfaces.msg import ParameterValue, ParameterType
        p_value = ParameterValue(type=ParameterType.PARAMETER_STRING_ARRAY, string_array_value=string_array)
