  ...
  const ControllerSpec & controller) const
{
  ...
  // Debug: show controller custom node options args BEFORE we merge cm args.
  {
    std::string pre;
    for (const auto & a : node_options_arguments)
    {
      pre.append(a);
      pre.append(" ");
    }
    RCLCPP_DEBUG(
      get_logger(), "Controller '%s' custom node_options initial args: %s",
      controller.info.name.c_str(), pre.c_str());
  }

  // Inherit some arguments from controller_manager node options, but be defensive:
  // We've observed malformed global args like '--params-file -p' (where '-p' is the
  // *spawner* short option) being forwarded here, which makes controller node creation
  // fail with RCLInvalidROSArgsError.
  const auto & cm_args = cm_node_options_.arguments();

  // Debug: show cm_node_options_ args that are being considered for inheritance.
  {
    std::string cm;
    for (const auto & a : cm_args)
    {
      cm.append(a);
      cm.append(" ");
    }
    RCLCPP_DEBUG(get_logger(), "controller_manager inherited cm_node_options args: %s", cm.c_str());
  }
  for (size_t i = 0; i < cm_args.size(); ++i)
  {
    const std::string & arg = cm_args[i];

    // Drop broken "--params-file" pairs coming from global args.
    if (arg == RCL_PARAM_FILE_FLAG)
    {
      const std::string next = (i + 1 < cm_args.size()) ? cm_args[i + 1] : std::string();
      if (next.empty() || next == "-p" || next == "--param-file" || (!next.empty() && next[0] == '-'))
      {
        RCLCPP_WARN(
          get_logger(),
          "Ignoring malformed global args '%s %s' while building node options for controller '%s'",
          arg.c_str(), next.c_str(), controller.info.name.c_str());
        if (i + 1 < cm_args.size())
        {
          ++i;  // skip value token
        }
  ...
  // Final safety: remove any malformed '--params-file -p' pair, regardless of source.
  for (size_t i = 0; i + 1 < node_options_arguments.size();)
  {
    if (node_options_arguments[i] == RCL_PARAM_FILE_FLAG && node_options_arguments[i + 1] == "-p")
    {
      RCLCPP_WARN(
        get_logger(),
        "Dropping malformed args '%s %s' from final controller node arguments for '%s'",
        node_options_arguments[i].c_str(), node_options_arguments[i + 1].c_str(),
        controller.info.name.c_str());
      node_options_arguments.erase(node_options_arguments.begin() + static_cast<long>(i),
        node_options_arguments.begin() + static_cast<long>(i) + 2);
      continue;
    }
    ++i;
  }

  // Remove empty tokens and stray "use_sim_time:=true"/"use_sim_time:=false" tokens.
  // Those must be passed via --param (RCL_PARAM_FLAG) to be valid ROS args.
  for (size_t i = 0; i < node_options_arguments.size();)
  {
    const auto & tok = node_options_arguments[i];
    if (tok.empty() || tok == "use_sim_time:=true" || tok == "use_sim_time:=false")
    {
      RCLCPP_WARN(
        get_logger(),
        "Dropping malformed token '%s' from final controller node arguments for '%s'",
        tok.c_str(), controller.info.name.c_str());
      node_options_arguments.erase(node_options_arguments.begin() + static_cast<long>(i));
      continue;
    }
    ++i;
  ...
  // Remove any remaining empty tokens anywhere (not just trailing). An empty token will be
  // reported as UnknownROSArgsError with argument '' by rclcpp.
  node_options_arguments.erase(
    std::remove_if(
      node_options_arguments.begin(), node_options_arguments.end(),
      [](const std::string & s) { return s.empty(); }),
    node_options_arguments.end());

  // Remove any '--ros-args' that ends up being the last token (after cleanup), or that is
  // followed by another '--ros-args'. Both cases create an empty/invalid ROS-args context.
  for (size_t i = 0; i < node_options_arguments.size();)
  {
    if (node_options_arguments[i] == RCL_ROS_ARGS_FLAG)
    {
      const bool last = (i + 1 >= node_options_arguments.size());
      const bool next_is_ros_args = (!last && node_options_arguments[i + 1] == RCL_ROS_ARGS_FLAG);
      if (last || next_is_ros_args)
      {
        RCLCPP_WARN(
          get_logger(), "Dropping stray '%s' from final controller node arguments for '%s'",
          RCL_ROS_ARGS_FLAG, controller.info.name.c_str());
        node_options_arguments.erase(node_options_arguments.begin() + static_cast<long>(i));
        continue;
      }
    }
  ...
  // Final pass: if anything appended another trailing '--ros-args', drop it.
  while (!node_options_arguments.empty() && node_options_arguments.back() == RCL_ROS_ARGS_FLAG)
  {
    RCLCPP_WARN(
      get_logger(), "Dropping trailing '%s' from final controller node arguments for '%s'",
      RCL_ROS_ARGS_FLAG, controller.info.name.c_str());
    node_options_arguments.pop_back();
  }

  // Trailing cleanup is now handled by the generic empty-token and stray '--ros-args' cleanup above.
  ...
  {
    // Defensive: guard against malformed values accidentally injected via parameters.
    // The most common failure observed is the short flag "-p" (from spawner CLI) being
    // stored as a params file path, which would lead to '--params-file -p' and a fatal
    // RCLInvalidROSArgsError during controller node initialization.
    if (parameters_file.empty() || parameters_file == "-p" || parameters_file == "--param-file")
    {
      RCLCPP_WARN(
        get_logger(),
        "Ignoring invalid params_file entry '%s' for controller '%s'", parameters_file.c_str(),
        controller.info.name.c_str());
      continue;
    }
  ...
  // ---------------------------------------------------------------------------
  // At this point all sources have been appended:
  //  - controller custom node_options
  //  - inherited controller_manager args
  //  - params_file entries
  //  - use_sim_time (as --param)
  //  - spawner-provided controller ros args
  // Any invalid/empty tokens added late must be cleaned *here*.
  auto is_blank = [](const std::string & s)
  {
    return std::all_of(s.begin(), s.end(), [](unsigned char c) { return std::isspace(c) != 0; });
  };

  // Drop any empty/whitespace-only tokens anywhere.
  for (size_t i = 0; i < node_options_arguments.size();)
  {
    if (node_options_arguments[i].empty() || is_blank(node_options_arguments[i]))
    {
      RCLCPP_WARN(
        get_logger(),
        "Dropping empty/blank token from final controller node arguments for '%s'", 
        controller.info.name.c_str());
      node_options_arguments.erase(node_options_arguments.begin() + static_cast<long>(i));
      continue;
    }
  ...
  // Drop any stray/duplicate/trailing '--ros-args' *after* all args are present.
  for (size_t i = 0; i < node_options_arguments.size();)
  {
    if (node_options_arguments[i] == RCL_ROS_ARGS_FLAG)
    {
  ...
  // Extra debug: show any empty tokens with their index (helps diagnose UnknownROSArgsError with '').
  for (size_t i = 0; i < node_options_arguments.size(); ++i)
  {
    if (node_options_arguments[i].empty())
    {
      RCLCPP_WARN(
        get_logger(), "Controller '%s' has empty token at args[%zu]", controller.info.name.c_str(),
        i);
    }
  }
  RCLCPP_INFO(
